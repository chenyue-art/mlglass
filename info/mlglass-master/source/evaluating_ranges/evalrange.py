...
def oracle(low, high, alg_low, alg_middle, alg_high):
    ...
    # 过滤掉包含 np.nan 的结果
    valid_perf = [p for p in perf if not np.isnan(p).any()]
    if valid_perf:
        evaluation = np.concatenate([np.mean(valid_perf, axis=0), np.std(valid_perf, axis=0)])
    else:
        evaluation = np.array([np.nan] * 14)  # 如果没有有效结果，返回包含 14 个 np.nan 的数组
    ...
    for res_range in [res_low, res_middle, res_high]:
        if res_range == None:
            ...
            valid_perf = [p for p in perf if not np.isnan(p).any()]
            if valid_perf:
                evaluation = np.concatenate([evaluation, np.mean(valid_perf, axis=0), np.std(valid_perf, axis=0)])
            else:
                evaluation = np.concatenate([evaluation, np.array([np.nan] * 7), np.array([np.nan] * 7)])
