...
def plot_violins(plot_data, relevances, filename):
    ...
    sns.violinplot(
        x='label', y='dist', hue='label',data=dt_plot,
        inner=None, palette='viridis', linewidth=0.7, cut=0,
        density_norm='width', bw_method='silverman', gridsize=1000
    )
    ...
    if relevances:
        norm = plt.Normalize(min(relevances), max(relevances))
        sm = plt.cm.ScalarMappable(cmap='viridis', norm=norm)
        sm.set_array([])  # 这一步是必要的
        ticks = np.linspace(min(relevances), max(relevances), 11)
        cbar = ax.figure.colorbar(sm, ax=ax,fraction=0.012, pad=0.04, ticks=ticks)
        cbar.ax.set_yticklabels(['{}%'.format(int(100 * v)) for v in ticks], ha='right')
...
if __name__ == '__main__':
    model_name = '{0}/rf/rf_ND300_final.model'.format(output_path)

    with open(model_name, 'rb') as f:
        rf = pickle.load(f)

    data = pd.read_csv(
        '{0}/data_ND300_dupl_rem.csv'.format(input_path)
    )
    ...
    filename_high = '{0}/interpretation/rf_vis_high_ND300.png'.format(output_path)
    filename_low = '{0}/interpretation/rf_vis_low_ND300.png'.format(output_path)
    filename_mid = '{0}/interpretation/rf_vis_mid_ND300.png'.format(output_path)
    ...
    if os.path.isfile(dp_h):
        ...
        with open(dp_h, 'wb') as f:
            pickle.dump(file=f, obj=(plot_data, relevances), protocol=pickle.HIGHEST_PROTOCOL)
