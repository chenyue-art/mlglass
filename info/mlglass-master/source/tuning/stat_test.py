...
def relative_deviation(obs, pred):
    # 复制数组避免修改原数据
    obs_copy = obs.copy().astype(float)
    # 找到 obs 中为 0 的位置，替换为一个极小值（如 1e-8），避免除 0
    zero_mask = obs_copy == 0
    obs_copy[zero_mask] = 1e-8
    # 计算相对偏差
    rel_dev = np.sum(np.abs(obs_copy - pred) / obs_copy) / len(obs) * 100
    return rel_dev
